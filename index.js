...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `
        <div style="background: white; border-radius: 10px; padding: 16px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); text-align: center;">
            <img src="${game.img}" alt="${game.name}" style="width: 100%; border-radius: 6px; margin-bottom: 12px;" />
            <h3>${game.name}</h3>
            <p>${game.description}</p>
            <p><strong>Backers:</strong> ${game.backers.toLocaleString()}</p>
        </div>
        ...
        grid.appendChild(gameCard.firstElementChild);
    }
...
const gameDescription = `
    ...
    for ${GAMES_JSON.length} games. Currently, ${numUnfundedGames} ${numUnfundedGames === 1 ? "game remains" : "games remain"} 
